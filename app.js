...
const STATS_KEY = "valueGuessStats_v2";
let stats = loadStats();
...
function luminanceToBin(L){
  const lightBin = 1 + Math.round(clamp01(L)*9); // 1 light → 10 dark
  return 11 - lightBin;
}
...
function linToSrgb(L){
  const u=L<=0.00313 ? 12.92*L : 1.055*Math.pow(L,1/2.4)-0.055;
...
function loadStats(){
  try{
    return JSON.parse(localStorage.getItem(STATS_KEY)) || {rounds:0,totalOff:0,lastOff:null};
  }catch{
